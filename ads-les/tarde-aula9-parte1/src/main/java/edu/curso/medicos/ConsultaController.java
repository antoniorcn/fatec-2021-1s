...
//	@Autowired
	...
	@RequestMapping(value="/consultas/{pagina}/{size}",
			method=RequestMethod.GET,
			produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public ResponseEntity<Collection<Consulta>> listaConsultas(HttpServletResponse response,
															   @PathVariable("pagina") Optional<Integer> numeroPagina,
															   @PathVariable("size") Optional<Integer> size) {
	    response.addHeader("Content-Type", "application/json");
		List<Consulta> lista = new ArrayList<>();
		int tamanhoPagina = 2;
		if (size.isPresent()) {
			tamanhoPagina = size.get();
		}
		if (numeroPagina.isPresent()) {
			Pageable paginacao = PageRequest.of(numeroPagina.get(), tamanhoPagina);
			Page<Consulta> paginaConsultas = consultaServicos.listarConsultasPaginacao(paginacao);
			lista = paginaConsultas.getContent();
		} else {
			lista = consultaServicos.listarConsultas();
